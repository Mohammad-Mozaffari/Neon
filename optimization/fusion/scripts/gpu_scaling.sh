...
do
    ...
    do
        ncu --kernel-regex execLambdaWithIterator_cuda -f --devices $PROFILE_GPU --set full -o ./compute/fusion_factor_${FUSION_FACTOR}_domain_size_${GRID_SIZE}_numGPUs_${GPU}_GPU_${PROFILE_GPU} --launch-count 1 "/home/mohammad/Neon/build/bin/fusion_map" --fusion_factor $FUSION_FACTOR --domain_size $GRID_SIZE --num_blocks $FUSION_FACTOR --times 1 --warmup 0 --eval_scalability 0 --gpus $GPUs --run_baseline 0
    done
done
# ncu --kernel-regex execLambdaWithIterator_cuda -f --devices 0,1,2,3 --set full -o ./compute/fusion_factor_${FUSION_FACTOR}_domain_size_${GRID_SIZE}_numGPUs_4_GPU_1234 --launch-count 1 "/home/mohammad/Neon/build/bin/fusion_map" --fusion_factor 1 --domain_size 256 --num_blocks 1 --times 1 --warmup 0 --eval_scalability 0 --gpus 0 1 2 3 --run_baseline 0
# ncu --kernel-regex execLambdaWithIterator_cuda --launch-skip 2722 --launch-count 1 "/home/mohammad/Neon/build/bin/fusion_map" --fusion_factor 1 --domain_size 256 --num_blocks 512 --times 10 --warmup 2 --eval_scalability 0 --gpus 0 1 2 --run_baseline 0
