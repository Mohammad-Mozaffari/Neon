...
std::vector<Neon::set::Container> make_fused_container(Neon::domain::dGrid::Field<T, 0> field_fused, T other_vals[], const int fusion_factor, const int flop_cnt, const int mem_access_cnt, const int NUM_BLOCKS)
{
	...
	for(int block = 0; block < NUM_BLOCKS; block++)
	{
		...
		else if(fusion_factor == 128 && flop_cnt == 1 && mem_access_cnt == 1)
			container_fused.push_back(mapContainerFused<Neon::domain::dGrid::Field<T, 0>, T, 128, 1, 1>(field_fused, other_vals));
		else if(fusion_factor == 128 && flop_cnt == 2 && mem_access_cnt == 1)
			container_fused.push_back(mapContainerFused<Neon::domain::dGrid::Field<T, 0>, T, 128, 2, 1>(field_fused, other_vals));
		else if(fusion_factor == 128 && flop_cnt == 4 && mem_access_cnt == 1)
			container_fused.push_back(mapContainerFused<Neon::domain::dGrid::Field<T, 0>, T, 128, 4, 1>(field_fused, other_vals));
		else if(fusion_factor == 128 && flop_cnt == 8 && mem_access_cnt == 1)
			container_fused.push_back(mapContainerFused<Neon::domain::dGrid::Field<T, 0>, T, 128, 8, 1>(field_fused, other_vals));
		else if(fusion_factor == 256 && flop_cnt == 1 && mem_access_cnt == 1)
			container_fused.push_back(mapContainerFused<Neon::domain::dGrid::Field<T, 0>, T, 256, 1, 1>(field_fused, other_vals));
		else if(fusion_factor == 256 && flop_cnt == 2 && mem_access_cnt == 1)
			container_fused.push_back(mapContainerFused<Neon::domain::dGrid::Field<T, 0>, T, 256, 2, 1>(field_fused, other_vals));
		else if(fusion_factor == 256 && flop_cnt == 4 && mem_access_cnt == 1)
			container_fused.push_back(mapContainerFused<Neon::domain::dGrid::Field<T, 0>, T, 256, 4, 1>(field_fused, other_vals));
		else if(fusion_factor == 256 && flop_cnt == 8 && mem_access_cnt == 1)
			container_fused.push_back(mapContainerFused<Neon::domain::dGrid::Field<T, 0>, T, 256, 8, 1>(field_fused, other_vals));
		else if(fusion_factor == 512 && flop_cnt == 1 && mem_access_cnt == 1)
			container_fused.push_back(mapContainerFused<Neon::domain::dGrid::Field<T, 0>, T, 512, 1, 1>(field_fused, other_vals));
		else if(fusion_factor == 512 && flop_cnt == 2 && mem_access_cnt == 1)
			container_fused.push_back(mapContainerFused<Neon::domain::dGrid::Field<T, 0>, T, 512, 2, 1>(field_fused, other_vals));
		else if(fusion_factor == 512 && flop_cnt == 4 && mem_access_cnt == 1)
			container_fused.push_back(mapContainerFused<Neon::domain::dGrid::Field<T, 0>, T, 512, 4, 1>(field_fused, other_vals));
		else if(fusion_factor == 512 && flop_cnt == 8 && mem_access_cnt == 1)
			container_fused.push_back(mapContainerFused<Neon::domain::dGrid::Field<T, 0>, T, 512, 8, 1>(field_fused, other_vals));
	}
...
std::vector<double> mapFusionPerfTest(bool generate_report = true, std::vector<int> devices = {0}, int domain_size = 1, int fusion_factor = 1, int flop_cnt = 1, int mem_access_cnt = 1, int max_num_blocks = 1, bool test_baseline = true)
{
    ...
    std::cout << "Starting Domain Size: " << domain_size << ", Fusion Factor: " << fusion_factor << ", Flop Count: " << flop_cnt << ", Memory Access Count: " << mem_access_cnt << ", Number of Blocks: " << num_blocks << std::endl;
